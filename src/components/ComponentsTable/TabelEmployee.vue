    ...
    <table class="w-full">
        <thead>
            <tr class="border-b border-gray-600">
                ...
                <th class="p-4 cursor-pointer border-r border-gray-600" @click="changeSort('work_type')">Тип праці</th>
                <th class="p-4 cursor-pointer border-r border-gray-600" @click="changeSort('start_date')">Дата прийому</th>
                <th class="p-4 cursor-pointer border-r border-gray-600" @click="changeSort('invalidity')">Інвалідність</th>
        ...
        <tbody>
            ...
            v-for="item in employees" :key="item.id">
                ...
                <td class="p-4 border-r border-b border-gray-600">{{ item.work_type }}</td>
                <td class="p-4 border-r border-b border-gray-600">{{ item.start_date }}</td>
                <td class="p-4 border-r border-b border-gray-600">{{ item.invalidity ? '' : item.invalidity || '――' }}</td>
                <td class="p-4 border-r border-b border-gray-600">{{ item.VBO ? '' : item.VBO || '――'}}</td>
                <td class="p-4 border-r border-b border-gray-600">{{ item.ZSU ? '' : item.ZSU || '――'}}</td>
