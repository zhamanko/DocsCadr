...
export default {
    ...
    mounted() {
        ...
        closeButton.addEventListener('click', this.closeSlideBar);
    },
    methods: {
        isActive(name) {
        ...
        linkClass(name) {
            return this.isActive(name) ? 'bg-[#303439] scale-110' : '';
        },
        closeSlideBar() {
            const slideBar = document.getElementById('slide-bar-left');
            const h1 = slideBar.querySelector('h1');
            if (slideBar) {
        ...
        toggleWidth() {
            const slideBar = document.getElementById('slide-bar-left');
            if (slideBar) {
                slideBar.classList.toggle('w-64');
                slideBar.classList.toggle('w-24');
            }
        }
    }
};
...
<template>
    <div id="slide-bar-left" class="bg-[#1d1e20] w-24 min-h-screen p-4 shadow-lg flex flex-col items-center gap-2">
        <div class="fixed">
